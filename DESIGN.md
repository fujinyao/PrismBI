...
| `/admin/sso` | **规划** | — | SSO/OIDC 配置 API 已有基础存储, 前端页面未完成 |
...
| 路径 | 说明 | 限制 |
|------|------|------|
| `/mobile` | 移动端首页 (项目列表) | 只读 |
| `/mobile/home` | 问答首页 | 只读 |
| `/mobile/home/[threadId]` | 对话线程 | 只读, 可提问 |
| `/mobile/dashboard` | Dashboard | 只读 |
| `/mobile/dashboard/[id]` | 单个仪表盘 | 只读, 图表可交互 |
| `/mobile/profile` | 个人资料 / API Token | 只读 |
| `/mobile/modeling` | 只读模型列表 (按数据源分组) | 仅查看, 不可编辑 |
| `/mobile/modeling/:modelName` | 单个模型字段详情 | 仅查看字段/类型/描述 |
...
- **WebSocket 安全**: `/ws/ask?token=xxx` 使用**短寿命 WebSocket 专用 token** (从 JWT 派生, 有效期 5 分钟), 避免长期 JWT 在 URL 日志中暴露
- **查询参数风格**: 统一使用 **camelCase** (如 `?projectId`, `?sourceLayer`); 列表端点支持 `?page` 与 `?pageSize` 分页
- **常见错误码**:
...
(public) POST /api/auth/register    { username, password, displayName } → { user }
...
```
GET    /api/admin/users                     → 用户列表 (支持 ?search, ?status, ?role, &page, &pageSize)
POST   /api/admin/users                     → 创建用户 { username, password, displayName, email, status } → { id }
GET    /api/admin/users/:id                 → 用户详情 (含角色分配)
PUT    /api/admin/users/:id                 → 更新用户 { displayName, email, status }
POST   /api/admin/users/:id/reset-password  → 管理员重置密码 { newPassword }
POST   /api/admin/users/:id/deactivate      → 禁用用户 (status=INACTIVE)
DELETE /api/admin/users/:id                 → 永久删除用户
POST   /api/admin/users/:id/roles           → 分配角色 { roleId, projectId?, expiresAt? }
...
PUT    /api/admin/roles/:id/permissions     → 批量更新角色权限 { permissionIds[] } → { success }

GET    /api/admin/audit-logs                → 审计日志 (?eventType, ?userId, ?from, ?to, &page, &pageSize)
POST   /api/admin/audit-logs/export         → 导出审计日志 { format: "csv"|"json" } → 文件下载

GET    /api/admin/sso                       → SSO 配置
PUT    /api/admin/sso                       → 更新 SSO 配置 { provider, clientId, clientSecret, issuerUrl, mappingRules }
```
...
POST   /api/projects                → 创建项目 { name, type, connectionInfo } → { id }
GET    /api/projects/:id            → 项目详情
PUT    /api/projects/:id            → 更新项目 { name?, displayName?, connectionInfo?, language? }
DELETE /api/projects/:id            → 删除项目
POST   /api/projects/:id/switch     → 切换当前项目
POST   /api/projects/:id/export     → 导出项目 { format: "yaml"|"json"|"csv" } → 文件下载
POST   /api/projects/import         → 导入项目 (文件上传) → { id }

#### 项目成员管理

```
GET    /api/projects/:id/members          → 项目成员列表
POST   /api/projects/:id/members          → 添加成员 { userId, roleId, expiresAt? }
PUT    /api/projects/:id/members/:memberId  → 修改成员角色 { roleId }
...
```
POST /api/query           { sql, projectId, limit?, dryRun? } → { columns, rows, totalRows }
POST /api/dry-plan        { sql, projectId } → { plannedSql, modelRefs }
```
...
**WebSocket 主通道**:
```
WebSocket: /ws/ask?token=xxx
  → { type: "ping" }
  ← { type: "pong" }
  → { type: "ask", request_id, question, thread_id?, previous_questions?, language?, preview_row_limit?, temporary? }
  ← { type: "delta", content_type: "state", content: "running", request_id }
  ← { type: "result", data: { thread_id, response, summary, sql }, request_id }
  ← { type: "error", message, request_id? }
```

当前 WebSocket 实现说明:
- `/ws/ask` 使用 query token 鉴权, 复用 HTTP JWT/API token 校验、session revocation 和 API token scope/RBAC 逻辑; 无 token 或无效 token 直接关闭连接。
...
- 当前 WebSocket 仍是“状态 delta + 最终 result”的最小流式实现, 尚未把 LLM token、SQL 生成、图表生成拆成逐 token/逐阶段 delta。
...
REST 备选 (非流式, 兜底):
```
POST /api/ask             { question, threadId, previousQuestions? } → { sql, summary, threadId, response }
```

当前实现状态:
- `/api/ask` 已挂载并复用统一 Ask Service; `/api/ask/stream` 当前为同一非流式结果的兼容入口, 真 SSE 分块仍待实现。
...
```
POST /api/ask/stream      { question, threadId, previousQuestions? } → SSE stream (event: delta / result / error)
```

线程:
POST   /api/threads                   → { thread } { projectId, summary? }
GET    /api/threads                   → 线程列表 (?projectId, &page, &pageSize)
...
```
GET    /api/diagram               → { models, views, relations, calculatedFields }
GET    /api/models                → 模型列表
GET    /api/models/:id            → 模型详情
POST   /api/models                → 创建模型 { name, displayName, description?, tableReference, columns, sourceId? } → { id }
PUT    /api/models/:id            → 更新模型 { name?, displayName?, description?, columns? }
DELETE /api/models/:id            → 删除模型

GET    /api/views                 → 视图列表
POST   /api/views                 → 创建视图 { name, displayName, description?, modelId, columns, relation? } → { id }
PUT    /api/views/:id             → 更新视图 { name?, displayName?, description?, columns? }
DELETE /api/views/:id             → 删除视图

POST   /api/relations             → 创建关系 { name, description?, sourceModelId, sourceColumn, targetModelId, targetColumn, type } → { id }
PUT    /api/relations/:id         → 更新关系 { name?, description?, sourceColumn?, targetColumn?, type? }
DELETE /api/relations/:id         → 删除关系

POST   /api/calculated-fields     → 创建计算字段 { name, modelId, expression, type, displayName?, description? } → { id }
PUT    /api/calculated-fields/:id → 更新 { name?, expression?, displayName?, description?, type? }
DELETE /api/calculated-fields/:id → 删除
```
...
POST   /api/dashboards            → 创建 { name, projectId, cacheEnabled?, schedule? } → { id }
PUT    /api/dashboards/:id        → 更新 { name?, cacheEnabled?, schedule? }
DELETE /api/dashboards/:id        → 删除

GET    /api/dashboards/:id/items           → 面板列表
POST   /api/dashboards/:id/items           → 新增面板 { title?, displayName?, chartConfig?, dataSource?, responseId?, type? } → { id }
PUT    /api/dashboards/:id/items/:itemId   → 更新面板 { title?, chartConfig?, dataSource?, type? }
DELETE /api/dashboards/:id/items/:itemId   → 删除面板
PUT    /api/dashboards/items/layouts       → 批量更新布局 { layouts: [{ itemId, x, y, w, h }] }
POST   /api/dashboards/items/:id/preview   → 预览数据 → { columns, rows }
POST   /api/dashboards/:id/schedule        → 设置缓存计划 → { scheduleId }
```
...
```
GET    /api/knowledge/instructions         → 指令列表 (?projectId, &search, &sort, &page, &pageSize)
POST   /api/knowledge/instructions         → 创建 { projectId, content, priority? } → { id }
PUT    /api/knowledge/instructions/:id     → 更新 { content?, priority? }
DELETE /api/knowledge/instructions/:id     → 删除

GET    /api/knowledge/sql-pairs            → SQL Pair 列表 (?projectId, &search, &sort, &page, &pageSize)
POST   /api/knowledge/sql-pairs            → 创建 { projectId, question, sql, category? } → { id }
PUT    /api/knowledge/sql-pairs/:id        → 更新 { question?, sql?, category? }
...
```
## 问题推荐 (核心 API)
GET  /api/recommendations/:projectId               → { data: [{ title, category, scope, source_type, confidence, metadata }] }
     ?max_results=5&types=expand,drilldown,compare

## Schema 推荐 (层 0: 冷启动)
当前前端 onboarding 复用项目推荐列表形成冷启动问题集合; 独立 onboarding route 仍未实现。

## 自学习 Catalog (层 2)
GET    /api/recommendations/:projectId/catalog       → { data: [{ id, question, sql, metadata, verified, frequency }] }
POST   /api/recommendations/:projectId/catalog       → { id }
PUT    /api/recommendations/:projectId/catalog/:id   → { success }
DELETE /api/recommendations/:projectId/catalog/:id   → { success }

## 用户偏好 Hints (Odin 式)
GET    /api/recommendations/:projectId/hints         → { data: [{ id, hint_text, source_query, confidence }] }
POST   /api/recommendations/:projectId/hints         → { id }
PUT    /api/recommendations/:projectId/hints/:id     → { success }
DELETE /api/recommendations/:projectId/hints/:id     → { success }

## 推荐反馈 (隐式 + 显式)
POST /api/recommendations/:projectId/dismiss/:recommendationId → { success }

## 推荐评分 (核心评分 API)
POST /api/recommendations/:projectId/rate/:recommendationId → { id, recommendation_id, rating, comment }
     { rating: 1-5, comment? }

## 查看评分历史
GET  /api/recommendations/:projectId/scores        → { data: [...] }
...
GET  /api/recommendations/:recommendationId/rating  → { avg_score, total_ratings, distribution }
PUT  /api/settings/recommendations                 → { success }
     { schemaWeight, sessionWeight, userWeight, projectWeight, globalWeight, llmWeight, noveltyWeight, scoreWeight, scoreLearningRate, scoreHalfLife, lowScoreThreshold, consecutiveLowAlert, autoRecover }
```

#### 图表

```
GET  /api/chart           ?question=...&sql=...&sampleSize= → { vegaSpec, chartType }
```

#### 内存层 (LanceDB)

```
GET  /api/memories/search   ?query=...&type=... → { results }
POST /api/memories/store    { type, content } → { id }
GET  /api/memories/list     ?type=... → { items }
POST /api/memories/forget   { id } → { success }
```
...
PUT  /api/settings/branding       { appName, logo, icon } → { success }
PUT  /api/settings/theme          { mode: "light"|"dark", primaryColor } → { success }
PUT  /api/settings/llm            { provider, apiKey, model, endpoint?, maxTokens?, temperature?, extraParams? } → { success }
POST /api/settings/llm/test       { provider, apiKey, model, endpoint? } → { success, latencyMs?, error? }
PUT  /api/settings/general        { language, defaultPage, telemetry } → { success }
...
POST   /api/system/datasources/:id/test               → 测试连接 → { success, latencyMs?, error? }

## 项目级数据源绑定 (多对多)
GET    /api/projects/:id/datasources                 → 项目绑定的数据源列表
POST   /api/projects/:id/datasources                 → 绑定已有系统数据源 { datasourceId, alias? } → { bindingId }
DELETE /api/projects/:id/datasources/:bindingId      → 解绑数据源 (不影响系统注册)
...
## 数据源元数据发现
GET    /api/projects/:id/datasources/:bindingId/tables  → 列出该数据源的可用表
POST   /api/projects/:id/datasources/:bindingId/sync    → 同步表结构变化 → { tablesDiscovered, tablesRemoved }
```
...
UI (WebSocket Client)
  │
  │ connect: /ws/ask?token=xxx
  │
  │ send:  { type: "ask", question: "上月销售额多少?", threadId: 1 }
  │
...
修复再验证使用 `_repair_sql` 尝试修复后, 对输出再次运行原验证器: 若通过则立即返回修复结果; 若仍失败则根据 `tier` 映射的 `max_retries` (tier1=1, tier2=2, tier3=3) 决定是否重试整个生成。重试时, 前次失败原因 (孤儿 CTE 错误、未知列名等) 作为 error feedback 注入 LLM prompt, 指导下一次修正。
...
定义于 `ask_service.py:40`。`max_retries` 对应各 tier 的最大 LLM 重试次数 (已接入重试循环)。`max_sub_questions` 限制 decompose-merge 子问题数 (超过截断)。`max_suggested_questions` 和 `metadata_summary_max_models` 限制无匹配引导时的推荐数和模型摘要数。`guidance_llm_available` 控制引导是否使用 LLM (False 时退化为模板引导)。`schema_pruning_enabled` 控制 Schema 剪枝是否启用。
...
- `max_retries` 在 ROUTER_CONFIG 中已定义但路由引擎尚未实现重试循环, 当前每个 tier 只调用一次 LLM。
- `fewshot_cot` tier-2 当前为 hint-based 提示, 尚未接入真实 few-shot example 检索和链式推理步骤。
- `_analyze_question` 结果未做会话级缓存, 同线程内连续提问会重复分析。
- 聚合一致性验证 (`_validate_sql_aggregation`) 不支持 position-based GROUP BY (如 `GROUP BY 1, 2`)。
- 后处理验证均为 warn-only (GROUP BY 完整性、聚合一致性) 或单次修复 (孤儿 CTE、列验证), 没有迭代修复循环。
...
| SSO/OIDC | 默认关闭, 配置占位 | `/api/admin/sso` 可保存配置且默认 `enabled=false`; 登录跳转/回调/自动建用户尚未实现, 仅超级管理员开启后生效 |
...
> 当前决策: SSO/OIDC 默认不开启。仅超级管理员根据企业实际情况配置并启用后, 登录页才显示 SSO 入口。当前状态: `metadata.settings['sso_config']` 和 `/api/admin/sso` 读写接口已实现并受 `sso:read/update` 权限保护, 默认 `enabled=false`; OIDC 登录按钮、回调、ID Token 校验、自动用户同步尚未实现。
...
/admin/sso                 → SSO 配置 (OIDC 端点, 客户端 ID, 映射规则) [API 已有, 页面规划]

/projects/:id/members      → 项目成员管理 (项目管理员)
...
| `PRISMBI_BACKUP_DIR` | `backend/data/backups` | DuckDB 备份目录 (规划) |
...
|------|-----------|------|
| `/mobile` | — | 移动首页 (项目列表) |
| `/mobile/home` | `/home` | 问答首页 (推荐问题 + 输入) |
| `/mobile/home/[threadId]` | `/home/[id]` | 对话线程 |
| `/mobile/dashboard` | `/home/dashboard` | 仪表盘列表 |
| `/mobile/dashboard/[id]` | — | 单个仪表盘 (全屏图表) |
| `/mobile/profile` | `/settings/profile` | 用户个人资料, API Token 管理 (只读) |

#### 移动端只读建模路由
移动端不提供建模编辑功能, 但在设置向导完成后可查看模型结构:
| 路径 | 说明 | 限制 |
|------|------|------|
| `/mobile/modeling` | 只读模型列表 (按数据源分组) | 仅查看, 不可编辑 |
| `/mobile/modeling/:modelName` | 单个模型字段详情 | 仅查看字段/类型/描述 |
...
```
mobile/
├── MobileShell.tsx          # 移动端壳 (TopNav + BottomTab + Content)
├── MobileTabBar.tsx         # 底部 Tab (Home / Dashboard / Profile)
├── ProjectList.tsx          # 项目选择列表
├── ThreadCard.tsx           # 线程卡片 (精简版)
├── CompactPrompt.tsx        # 紧凑输入框
├── CompactRecommendation.tsx # 紧凑推荐列表 (2-3 条, 可横向滑动)
├── CompactStarRating.tsx   # 触屏优化星级评分 (大点击区域)
├── ChartViewer.tsx          # 全屏图表查看器
